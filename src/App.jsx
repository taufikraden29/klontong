...
export default function App() {
  ...
  return (
    <div className="flex min-h-screen bg-white font-inter">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-6 md:gap-8 animate-fade-in items-start h-full">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 overflow-y-auto pr-1">
                {products.filter(p => (selectedCategory === 'Semua' || p.category === selectedCategory) && (p.name.toLowerCase().includes(searchTerm.toLowerCase()) || (p.barcode && p.barcode.includes(searchTerm)))).map(p => (
                  <button 
                    key={p.id} 
                    onClick={() => addToCart(p)} 
                    className={`card-minimal !p-3 md:!p-4 text-left flex flex-col h-full hover:border-primary/30 active:scale-95 transition-all ${p.stock <= 0 ? 'opacity-40 grayscale cursor-not-allowed' : 'bg-white'}`} 
                    disabled={p.stock <= 0}
                  >
                    ...
                    <div className="flex items-end justify-between gap-1 mt-auto">
                      ...
                      <div className={`text-[9px] md:text-[10px] font-bold px-2 py-0.5 rounded-full ${p.stock < 10 ? 'bg-red-50 text-red-600' : 'bg-neutral-50 text-neutral-400'}`}>Stok {p.stock}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <aside className="sticky top-10 flex flex-col gap-4 hidden xl:flex">
              <div className="card-minimal !p-0 flex h-[calc(100vh-120px)] flex-col overflow-hidden border-2 border-neutral-100 shadow-xl">
                <div className="border-b border-neutral-100 p-5 flex items-center justify-between bg-neutral-50"><h2 className="text-sm font-bold flex items-center gap-2"><ShoppingCart size={16} /> Keranjang</h2><button onClick={() => setCart([])} className="text-neutral-400 hover:text-red-500 transition-colors" title="Bersihkan Keranjang"><Eraser size={16} /></button></div>
                ...
                <div className="bg-neutral-50 p-5 border-t border-neutral-100 space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between text-[10px] font-bold text-neutral-400 uppercase tracking-wider"><span>Subtotal</span><span>{formatPrice(cartSubtotal)}</span></div>
                    <div className="flex items-center justify-between gap-4"><span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider">Diskon</span><input type="number" className="w-24 bg-white border border-neutral-200 text-right text-xs font-bold px-2 py-1 rounded" value={discount} onChange={(e) => setDiscount(Math.max(0, parseInt(e.target.value) || 0))} /></div>
                    <div className="flex justify-between text-base font-bold border-t border-neutral-200 pt-3 text-neutral-900 uppercase"><span>Total</span><span className="text-primary">{formatPrice(cartTotal)}</span></div>
                  </div>
            ...
            <button 
              ...
              className={`fixed bottom-20 right-4 z-40 flex h-14 md:h-16 items-center gap-3 rounded-full bg-neutral-900 px-5 md:px-6 text-white shadow-2xl lg:hidden transform transition-all active:scale-90 ${cart.length > 0 ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0 pointer-events-none'}`}
            >
              <div className="relative">
                <ShoppingCart size={22} />
                <span className="absolute -right-2 -top-2 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] font-bold ring-2 ring-neutral-900">{cart.length}</span>
              </div>
              <div className="flex flex-col items-start leading-none">
                 <span className="text-[8px] font-bold text-neutral-400 uppercase tracking-widest">Total Bayar</span>
                 <span className="text-sm md:text-base font-bold">{formatPrice(cartTotal)}</span>
              </div>
            </button>
          </div>
        )}

        {/* --- INVENTORY, LOGS, DEBTS, TRANS, SETTINGS tabs (Simplified Responsive) --- */}
        {['inventory', 'stock_logs', 'debts', 'transactions'].includes(activeTab) && (
          <div className="animate-fade-in max-w-6xl mx-auto">
             <div className="mb-6 md:mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h1 className="text-xl md:text-2xl font-bold capitalize">{activeTab.replace('_', ' ')}</h1>
                  <p className="text-xs md:text-sm text-neutral-400">Kelola data {activeTab} toko Anda.</p>
                </div>
             ...
             {/* Dynamic Search & Filter per Tab */}
             {activeTab === 'inventory' && (
                <div className="mb-6 flex flex-col md:flex-row gap-3">
                   <div className="relative flex-1">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" size={16} />
                      <input type="text" placeholder="Cari barang atau barcode..." className="input-minimal pl-10 py-3 text-sm" value={inventorySearch} onChange={e => setInventorySearch(e.target.value)} />
                   </div>
                   <div className="flex gap-2">
                      <select className="input-minimal flex-1 md:w-auto py-2 text-[10px] md:text-xs font-bold" value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)}>
                        <option value="Semua">Kategori</option>{categories.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                      <select className="input-minimal flex-1 md:w-auto py-2 text-[10px] md:text-xs font-bold" value={inventoryFilter} onChange={e => setInventoryFilter(e.target.value)}>
                        <option value="semua">Status</option><option value="rendah">Stok Rendah</option><option value="habis">Habis</option>
                      </select>
                   </div>
                </div>
             )}
             ...
             <div className="card-minimal !p-0 overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                   <table className="w-full text-left">
                             ...
                             {products.filter(p => {
                               ...
                               <tr key={p.id} className="hover:bg-neutral-50">
                                 <td className="px-4 md:px-6 py-4">
                                    <div className="font-bold text-neutral-800 uppercase truncate max-w-[120px] md:max-w-none">{p.name}</div>
                                    <div className="text-[9px] text-neutral-400 font-mono">{p.barcode || '-'}</div>
                                 </td>
                                 ...
                                 <td className="px-4 md:px-6 py-4 text-center md:text-left"><div className={`inline-block rounded-full px-2 py-0.5 font-bold text-[10px] ${p.stock <= 0 ? 'bg-red-100 text-red-700' : p.stock < 10 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>{p.stock} <span className="hidden md:inline">{p.unit}</span></div></td>
                                 <td className="px-4 md:px-6 py-4 text-right">
                                    <div className="flex justify-end gap-1 md:gap-2">
                                       <button onClick={() => { setOpnameTarget(p); setShowOpnameModal(true); }} className="p-1.5 md:p-2 rounded-md hover:bg-neutral-100 text-neutral-400" title="Opname"><Settings2 size={14} md:size={16} /></button>
                                       <button onClick={() => { setEditingProduct(p); setShowProductModal(true); }} className="p-1.5 md:p-2 rounded-md hover:bg-neutral-100 text-neutral-400" title="Edit"><Edit2 size={14} md:size={16} /></button>
                                    </div>
                                 </td>
                               </tr>
                             ))}
                          ...
                          <tbody className="divide-y divide-neutral-100 text-xs">
                             {transactions.map(t => (
                               <tr key={t.id} className="hover:bg-neutral-50">
                                 <td className="px-4 md:px-6 py-4 text-neutral-400 text-[10px]">{t.date}</td>
                      ...
                      {/* ... other tables follow similar responsive patterns ... */}
                   </table>
                        ...
                        {stockLogs.map(log => (
                          <div key={log.id} className="flex items-center justify-between p-3 bg-neutral-50 rounded-xl border border-neutral-100">
                             <div className="flex-1 min-w-0">
                               <div className="text-[11px] font-bold text-neutral-800 uppercase truncate">{log.productName}</div>
                               <div className="text-[9px] text-neutral-400 uppercase tracking-tighter">{log.date} • {log.reason}</div>
                             </div>
                             <div className={`text-[11px] font-bold ${log.type === 'IN' ? 'text-green-600' : log.type === 'OUT' ? 'text-red-600' : 'text-blue-600'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="card-minimal !p-5">
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={exportAllData} className="btn-secondary justify-start gap-3 h-12 px-4 text-xs font-bold"><Download size={16} /> CADANGKAN DATA (.json)</button>
                  <label className="btn-secondary justify-start gap-3 h-12 px-4 text-xs font-bold cursor-pointer"><Upload size={16} /> PULIHKAN DATA<input type="file" className="hidden" accept=".json" onChange={importAllData} /></label>
                </div>
              </div>
              <div className="card-minimal !p-5">
                ...
                <div className="space-y-4">
                   ...
                   <div className="flex flex-wrap gap-2">
                      {categories.map(c => (<span key={c} className="flex items-center gap-2 rounded-full bg-neutral-100 px-3 py-1.5 text-[10px] font-bold text-neutral-600 border border-neutral-200">{c} <button onClick={() => deleteCategory(c)} className="text-neutral-300 hover:text-red-500"><X size={12} /></button></span>))}
                   </div>
                </div>
              </div>
            </div>
      ...
      {/* --- MODALS (Responsive Enhancements) --- */}
      ...
      {showMobileCart && (
        <div className="fixed inset-0 z-[100] flex items-end bg-neutral-900/60 backdrop-blur-sm animate-fade-in lg:hidden">
          <div className="w-full bg-white rounded-t-[2rem] p-6 animate-fade-in-up max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-6 shrink-0">
              ...
              <button onClick={() => setShowMobileCart(false)} className="p-2 rounded-full bg-neutral-100 text-neutral-400 active:scale-90"><X size={20} /></button>
            </div>
            ...
            <div className="bg-neutral-900 p-6 rounded-[1.5rem] text-white shrink-0 shadow-xl">
              ...
              <button 
                ...
                className="btn-primary w-full h-14 font-bold text-sm tracking-widest active:scale-95"
              >
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-6 bg-neutral-900/60 backdrop-blur-sm animate-fade-in">
          <div className="card-minimal w-full max-w-lg !p-6 md:!p-8 relative bg-white shadow-2xl rounded-t-[2rem] md:rounded-[2rem] h-[90vh] md:h-auto overflow-y-auto">
            <button onClick={() => setShowPaymentModal(false)} className="absolute top-6 right-6 text-neutral-300 hover:text-neutral-600 hidden md:block"><X size={20} /></button>
            <div className="md:hidden h-1.5 w-12 bg-neutral-200 rounded-full mx-auto mb-6"></div>
            
            <h2 className="text-xl font-bold mb-6 text-center md:text-left">Selesaikan Pembayaran</h2>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-1 gap-2">
                      {[
                        ...
                        { id: 'QRIS/Transfer', icon: QrCode },
                        { id: 'Hutang', icon: Users }
                      ].map(m => (
                        <button 
                          key={m.id} 
                          onClick={() => setPaymentMethod(m.id)} 
                          className={`flex items-center justify-center md:justify-start gap-3 px-4 py-3 rounded-xl text-[10px] md:text-xs font-bold transition-all border-2 ${paymentMethod === m.id ? 'bg-primary text-white border-primary shadow-md' : 'bg-white text-neutral-400 border-neutral-100 hover:border-neutral-200'}`}
                        >
                          <m.icon size={16} />
                          {m.id}
                        </button>
      ...
      {/* ... Other modals like Restock, Opname, Receipt (Responsive check) ... */}
        ...
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-6 bg-neutral-900/60 backdrop-blur-sm animate-fade-in">
           <form onSubmit={finalizeOpname} className="card-minimal w-full max-w-md !p-6 md:!p-8 relative bg-white rounded-t-[2rem] md:rounded-[2rem]">
              ...
              <div className="mb-6 p-4 bg-neutral-50 rounded-2xl">
                 <div className="text-[10px] font-bold text-neutral-400 uppercase">Nama Produk</div>
                 <div className="text-sm font-bold uppercase">{opnameTarget.name}</div>
                 <div className="text-[10px] text-neutral-500 mt-1 uppercase">Stok Sistem: <span className="font-bold">{opnameTarget.stock} {opnameTarget.unit}</span></div>
              </div>
              <div className="space-y-4 mb-8">
                 <div>
                    <label className="text-[10px] font-bold uppercase text-neutral-400 block mb-2 tracking-wider">Stok Sebenarnya</label>
                    <input name="newStock" type="number" required className="input-minimal h-12 text-lg font-bold" defaultValue={opnameTarget.stock} autoFocus />
                 </div>
                 <div>
                    <label className="text-[10px] font-bold uppercase text-neutral-400 block mb-2 tracking-wider">Pilih Alasan</label>
                    <select name="reason" className="input-minimal h-12 text-sm font-bold">
                       <option value="Opname Rutin">Opname Rutin</option><option value="Barang Rusak">Barang Rusak</option><option value="Bonus Distributor">Bonus Distributor</option>
                    </select>
                 </div>
              </div>
              <button className="btn-primary w-full h-14 font-bold tracking-widest active:scale-95">SIMPAN PERUBAHAN</button>
           </form>
        </div>
      )}
      ...
      {showScannerModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-neutral-900/90 backdrop-blur-md animate-fade-in">
          <div className="w-full max-w-sm bg-white rounded-[2rem] p-6 relative overflow-hidden">
             <button onClick={() => setShowScannerModal(false)} className="absolute top-4 right-4 z-10 p-2 bg-white/80 rounded-full text-neutral-400"><X size={20} /></button>
             ...
             <div className="mt-4 text-center">
                <p className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest animate-pulse">Arahkan kamera ke barcode produk</p>
             </div>
          </div>
        </div>
      )}
      ...
      <div className="fixed top-4 md:top-auto md:bottom-20 right-4 left-4 md:left-auto md:right-10 z-[300] flex flex-col gap-3 pointer-events-none items-center md:items-end">
        {notifications.map(n => (
          <div key={n.id} className="pointer-events-auto flex items-center gap-3 bg-neutral-900 text-white rounded-2xl px-6 py-4 shadow-2xl animate-fade-in text-[11px] font-bold min-w-[200px] border border-white/10">
             {n.type === 'success' ? <CheckCircle2 size={16} className="text-green-400" /> : <AlertTriangle size={16} className="text-red-400" />}
             <span>{n.message}</span>
          </div>
        ))}
      </div>
      ...
      {showReceiptModal && lastReceipt && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-neutral-900/60 backdrop-blur-md no-print animate-fade-in">
          <div className="w-full max-w-xs bg-white rounded-[2rem] overflow-hidden shadow-2xl flex flex-col">
            <div id="receipt-content" className="bg-white p-6 md:p-8 text-[10px] font-mono leading-tight text-neutral-900 overflow-y-auto">
              ...
              <div className="mb-4 flex justify-between font-bold border-b border-neutral-100 pb-2 text-[8px]">
                 <span>TRX {lastReceipt.id.slice(-6)}</span>
              ...
              <div className="space-y-3 mb-6 border-b border-dashed border-neutral-300 pb-4 text-left">
                {lastReceipt.items.map(i => (
                  <div key={i.id}>
                    <div className="flex justify-between font-bold uppercase"><span>{i.name}</span><span>{formatPrice(i.quantity * i.price)}</span></div>
                    <div className="flex justify-between text-neutral-500 text-[7px] uppercase">
                       <span>{i.quantity} x {formatPrice(i.price)}</span>
                       {i.itemDiscount > 0 && <span>Disc -{formatPrice(i.itemDiscount)}</span>}
                    </div>
                  </div>
                ))}
              </div>
